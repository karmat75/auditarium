...
internal sealed class ApplicationSettingResolver(AuditariumDbContext db, IConfiguration configuration) : IApplicationSettingResolver
{
    public async Task<EffectiveSetting> GetAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        if (definition.UiEditable)
        {
            ...
            if (stored is not null && TryRead(stored.SerializedValue, definition, out var value)) return new EffectiveSetting(key, value, SettingValueSource.Database, false);
        }
        ...
        return new EffectiveSetting(key, definition.DefaultJson, SettingValueSource.Default, false);
    }
