...
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddAuditariumPersistence(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddDbContext<AuditariumDbContext>((_, o) => { if (database.Provider.Equals("PostgreSQL", StringComparison.OrdinalIgnoreCase)) o.UseNpgsql(database.ConnectionString, x => x.MigrationsAssembly("Auditarium.Dal.PostgreSql.Migrations").MigrationsHistoryTable("__EFMigrationsHistory", "auditarium").CommandTimeout(database.BootstrapTimeoutSeconds)); else if (database.Provider.Equals("SqlServer", StringComparison.OrdinalIgnoreCase)) o.UseSqlServer(database.ConnectionString, x => x.MigrationsAssembly("Auditarium.Dal.SqlServer.Migrations").MigrationsHistoryTable("__EFMigrationsHistory", "auditarium").CommandTimeout(database.BootstrapTimeoutSeconds)); else throw new InvalidOperationException("Auditarium:Database:Provider must be PostgreSQL or SqlServer."); });
