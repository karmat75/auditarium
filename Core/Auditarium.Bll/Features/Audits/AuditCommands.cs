...
[RequiresPermission("Audits.Answer")] public sealed record AnswerAuditQuestionCommand(long AuditId, long AuditQuestionId, AuditQuestionResult Result, string? Comment, string? Evidence, long ConcurrencyVersion) : IRequest<Result>;
[RequiresPermission("Audits.Answer")] public sealed record ResetAuditQuestionCommand(long AuditId, long AuditQuestionId, long ConcurrencyVersion) : IRequest<Result>;
    ...
    IRequestHandler<CreateAuditUnitCommand, Result<long>>, IRequestHandler<UpdateAuditUnitCommand, Result>, IRequestHandler<DeleteAuditUnitCommand, Result>, IRequestHandler<CreateAuditCommand, Result<long>>, IRequestHandler<UpdateAuditDraftCommand, Result>, IRequestHandler<GetAuditPreviewQuery, Result<AuditPreview>>, IRequestHandler<PublishAuditCommand, Result>, IRequestHandler<ClaimAuditCommand, Result>, IRequestHandler<ReleaseOwnAuditCommand, Result>, IRequestHandler<AssignAuditorCommand, Result>, IRequestHandler<AnswerAuditQuestionCommand, Result>, IRequestHandler<ResetAuditQuestionCommand, Result>, IRequestHandler<FinalizeAuditCommand, Result>, IRequestHandler<CancelAuditCommand, Result>, IRequestHandler<ReopenAuditCommand, Result>, IRequestHandler<CreateAuditRepeatCommand, Result<long>>, IRequestHandler<DeleteAuditCommand, Result>
{
    ...
    {
        var audit = await db.Audits.SingleOrDefaultAsync(x => x.AuditId == m.AuditId, ct); if (audit is null) return NotFound("AUDIT.NOT_FOUND"); if (audit.ConcurrencyVersion != m.ConcurrencyVersion) return Conflict("AUDIT.CONCURRENCY_CONFLICT"); var access = await CanEditAsync(audit, ct); if (access is not null) return Fail(access, ErrorType.Forbidden); if (audit.AuditState != AuditState.InProgress) return Conflict("AUDIT.NOT_IN_PROGRESS"); var questions = await QuestionsForAuditAsync(audit.AuditId, ct); if (questions.Any(x => x.Result is null)) return Fail("AUDIT.FINALIZATION_INCOMPLETE"); var settings = Deserialize(audit.AuditSettings); if (questions.Any(x => !CompliesWithResponsePolicy(x, settings))) return Fail("AUDIT_QUESTION.RESPONSE_POLICY_VIOLATION"); audit.AuditState = AuditState.Finalized; audit.AssignedAuditorUserId = null; await db.SaveChangesAsync(ct); return Result.Success();
    }
    ...
    private static bool CompliesWithResponsePolicy(AuditQuestion question, AuditSettings settings)
    {
        if (question.Result is not { } result) return false;
        var rule = settings.ResponsePolicy[result];
        return (!rule.CommentRequired || !Empty(question.Comment)) && (!rule.EvidenceRequired || !Empty(question.Evidence));
    }
